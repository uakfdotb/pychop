...
public:
	...
	enum RankCode {
		CLAN_RECRUIT = 0,				// 0x00 first week member
		CLAN_PEON = 1,					// 0x01 peon
		CLAN_GRUNT = 2,					// 0x02 grunt
		CLAN_SHAMAN = 3,				// 0x03 shaman
		CLAN_CHIEF = 4					// 0x04 chieftain
	};
...
private:
	...
	BYTEARRAY m_ClanLastInviteCookie;	// set in RECEIVE_SID_CLANCREATIONINVITATION, SID_CLANINVITATIONRESPONSE
	bool m_ClanLastInviteSet;			// set in RECEIVE_SID_CLANCREATIONINVITATION, SID_CLANINVITATIONRESPONSE
...
public:
	...
	BYTEARRAY SEND_SID_AUTH_CHECK( BYTEARRAY clientToken, BYTEARRAY exeVersion, BYTEARRAY exeVersionHash, BYTEARRAY keyInfoROC, string exeInfo, string keyOwnerName );
	...
	BYTEARRAY SEND_SID_CLANINVITATION( string name );
	BYTEARRAY SEND_SID_CLANREMOVE( string name );
...
public:
	CIncomingChatEvent( CBNETProtocol :: IncomingChatEvent nChatEvent, bool nOperator, uint32_t nPing, string nUser, string nMessage );
	~CIncomingChatEvent( );

	CBNETProtocol :: IncomingChatEvent GetChatEvent( )	{ return m_ChatEvent; }
	bool GetOperator( )									{ return m_Operator; }
